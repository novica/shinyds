...
# Group several avatars in a stack. Each avatar is wrapped in an <li>
# automatically; `suffix` adds trailing text such as a count.
ds_avatar_stack(
  ds_avatar("AB"),
  ds_avatar("CD"),
  ds_avatar("EF"),
...
`ds_chip()` renders as a `<button>` and is display-only on its own.

For reactive multi-select filtering, wrap chips in `ds_chip_group()`. Selected chip
values are reported to Shiny as a character vector via `input[[inputId]]`:
...
Each chip needs a `value` attribute (passed via `...`) that becomes the reported value.
If `value` is omitted, the chip's text content is used instead. Disabled chips are
excluded from click handling automatically.
